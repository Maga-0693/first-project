...
но учитывайте, что после использования nextInt() сканер остается на конце считанной строки даже если она пустая
и её надо будет дочитать, чтобы перейти на следующую. Это можно сделать также методом nextLine(). В комментариях
оставлены фразы для печати и имена переменных.*/